...
local GlobalColors = {
    ButtonBGColor = Color3.fromRGB(45, 45, 45)
}

local Library = {}
Library.Tabs = {}

--// Fallback Icons Table (Replace with actual Roblox asset IDs and offsets)
local FallbackIcons = {
    ["48px"] = {
        ["home"] = {1234567890, {48, 48}, {0, 0}}, -- Replace 1234567890 with actual asset ID
        ["settings"] = {0987654321, {48, 48}, {0, 0}} -- Replace 0987654321 with actual asset ID
    }
}
...
local function loadIcons(url)
    ...
    if success then
        ...
        warn("Failed to load icons from " .. url .. ": " .. tostring(result))
        return nil
    end
end

-- Initialize Icons with fallback
local Icons = loadIcons("https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/refs/heads/main/icons.lua") or FallbackIcons
...
Version.Position = UDim2.new(0.5, 43, 0, 10)
...
function Library:CreateTab(options)
    ...
    Page.Name = Tab.Name
    Page.Size = UDim2.new(1, 0, 1, 0) -- Stretch to fill MainFrame
    Page.Position = UDim2.new(0, 0, 0, 0) -- Align to top-left of MainFrame
    Page.CanvasSize = UDim2.new(0, 0, 0, 0) -- Will be updated dynamically
    ...
    TabButton.Text = "" -- No text by default
    ...
    if options.Icon and type(options.Icon) == "string" then
        ...
        if iconSource:match("^lucide://") then
            local iconName = iconSource:sub(9) -- Remove "lucide://" prefix
            local iconButton = createIconButton(iconName, TabButton)
            if iconButton then
                iconButton.Position = UDim2.new(0.5, -10, 0.5, -10)
            else
                -- Fallback to text
    ...
    TabButton.MouseButton1Click:Connect(function()
        for _, otherTab in ipairs(Library.Tabs) do
            ...
            if otherTab.Button then
                TweenService:Create(otherTab.Button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(30, 30, 30)}):Play()
            end
        end
        Page.Visible = true
