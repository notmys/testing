...
local GlobalColors = {
    ...
    SliderBGColor = Color3.fromRGB(0, 22, 46),
    TextboxValueBoxBGColor = Color3.fromRGB(0, 22, 46)
}
local Library = {}
Library.Tabs = {}
...
function Library:CreateTab(options)
    ...
    -- Button and Slider creation inside page
    local buttons = {}
    local allSliderContainers = {}
    ...
    function Tab:CreateButton(options)
        ...
        button.Size = UDim2.new(1.03, 0, 0, 35)
        ...
        button.MouseEnter:Connect(
            function()
                ...
                local hoverColor = brighten(GlobalColors.ButtonBGColor, 20)
                TweenService:Create(
                    button,
                    TweenInfo.new(0.2),
                    {
        ...
        button.MouseLeave:Connect(
            function()
                TweenService:Create(
                    ...
                    TweenInfo.new(0.2),
                    {
                        BackgroundColor3 = GlobalColors.ButtonBGColor
                    }
                ):Play()
            end
        )
    ...
    function Tab:CreateSlider(options)
        if not options or type(options) ~= "table" then
            warn("Invalid options for CreateSlider")
            return
        end
        local container = Instance.new("Frame")
        container.Size = UDim2.new(1.03, 0, 0, 32)
        container.BackgroundColor3 = GlobalColors.SliderBGColor
        ...
        valueBox.Size = UDim2.new(0, 30, 0.7, 0)
        valueBox.Position = UDim2.new(0.74, 0, 0.15, 0)
        valueBox.AnchorPoint = Vector2.new(1, 0)
        valueBox.BackgroundTransparency = 0.8
        valueBox.BackgroundColor3 = GlobalColors.TextboxValueBoxBGColor
        ...
        slider.Size = UDim2.new(0.23, -5, 0.25, 0)
        slider.Position = UDim2.new(0.76, 0, 0.375, 0)
        ...
        fill.BackgroundColor3 = GlobalColors.SliderBGColor
        fill.ZIndex = 4
        ...
        local decimalPoints = options.DecimalPoints or 0
        local step = 10 ^ (-decimalPoints)
        local initialValue = options.Default or min
        ...
        local function roundToDecimal(val, decimals)
            local mult = 10 ^ decimals
            return math.floor(val * mult + 0.5) / mult
        end
        local value = clampValue(roundToDecimal(initialValue, decimalPoints))
        local function updateFillFromValue(val)
        ...
        local function updateSlider(x)
            ...
            local newValue = roundToDecimal(rawValue, decimalPoints)
            newValue = clampValue(newValue)
            if newValue ~= value then
                updateFillFromValue(newValue)
            end
        end
        ...
        valueBox.FocusLost:Connect(
            function()
                ...
                if num then
                    num = roundToDecimal(num, decimalPoints)
                    num = clampValue(num)
                    updateFillFromValue(num)
                else
        ...
        local function resizeValueBox()
            local textLength = #tostring(valueBox.Text)
            local padding = 10
            local targetWidth = math.clamp(textLength * 8 + padding, 30, 60)
            ...
            currentTween =
                ...
                {
                    Size = UDim2.new(0, targetWidth, 0.7, 0),
                    Position = UDim2.new(0.74, 0, 0.15, 0)
                }
            )
            currentTween:Play()
        end
        valueBox:GetPropertyChangedSignal("Text"):Connect(
            function()
                local pattern
                ...
                if not valueBox.Text:match(pattern) then
        ...
        table.insert(allSliderContainers, container)
        table.insert(allSliderFills, fill)
        updateFillFromValue(value)
        resizeValueBox()
        -- Update Page CanvasSize
        Page.CanvasSize = UDim2.new(0, 0, 0, Layout.AbsoluteContentSize.Y)
        return container
